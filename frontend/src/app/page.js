...
export default function Home() {
  ...
  // Auth state from centralized context
  const { isAuthenticated, userInfo, token } = useAuth();
  // Chat placeholders (to be wired to real chat history service)
  const [chatLoading] = useState(false);
  const [chatError] = useState(null);
  ...
  const createConversation = async () => ({ id: null });
  const updateConversation = async () => {};
  const saveMessageToDB = async () => {};
  const clearCurrentConversation = () => setCurrentConversation(null);
                          ...
                          onChange={async (event) => {
                            ...
                            try {
                              if (!isAuthenticated) {
                                const {
                                  useAlert,
                                } = require("@/components/AlertProvider");
                              ...
                              if (isRag) {
                                const selected = files.slice(0, 2);
                                const res = await ragUploadFiles(
                                  selected,
                                  token
                                );
                                setRagSessionId(res.session_id);
                                setRagMessage(
                                  `Processed ${res.files_processed} files · ${res.documents_extracted} chunks`
                                );
                                if (typeof window !== "undefined") {
                                  localStorage.setItem(
                                    "rag_session_id",
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (ragSessionId) {
                              try {
                                const stats = await ragGetSessionStats(
                                  ragSessionId
                                );
                                shouldUseRag =
                                  !!stats && (stats.document_count || 0) > 0;
                              } catch (e) {
                                console.warn(
                                  "Failed to read session stats; skipping RAG.",
                                  e
                                );
                                shouldUseRag = false;
                              }
                            ...
                            if (shouldUseRag) {
                              ...
                              fullJson = await ragRetrieveDetailed(
                                ragSessionId,
                                userPrompt,
                                5,
                                false,
                              ...
                              responseText =
                                fullJson?.results?.[0]?.content ||
                                "No results found.";
                            } else {
                              // Keep chat independent of RAG when no docs/session
                              responseText =
                                "Upload documents to use retrieval, or continue chatting.";
                              fullJson = {
                                reason: "rag_disabled",
                                sessionId: ragSessionId || null,
                              };
                            }
                            ...
                            if (
                              ...
                              const convId =
                                currentConversation?.id || activeConversationId;
                              const derivedTitle =
                                userPrompt.trim().slice(0, 40) || "New Chat";
                              try {
