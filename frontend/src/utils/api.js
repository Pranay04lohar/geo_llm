...
const CHAT_API_BASE =
  process.env.NEXT_PUBLIC_CHAT_API_BASE_URL || "http://localhost:8010/api/v1";
...
export async function uploadFiles(files) {
  ...
  if (res.status === 403)
    throw new Error(
      "Registration required: Please complete your profile to upload files."
    );
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
...
export async function getSessionStats(sessionId) {
  ...
  if (res.status === 401) {
    triggerAlert("error", "Sign in required", "Please sign in.");
    throw new Error("Unauthorized: Please sign in.");
  }
  if (res.status === 403)
    throw new Error("Registration required: Please complete your profile.");
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
...
export async function getChatConversation(conversationId) {
  const headers = await withAuthHeaders();
  const res = await fetch(`${CHAT_API_BASE}/conversations/${conversationId}`, {
    headers,
  });
  if (res.status === 401) {
    triggerAlert(
      "error",
      "Sign in required",
      "Please sign in to view conversation."
    );
    throw new Error("Unauthorized: Please sign in.");
  }
...
export async function getChatMessages(conversationId, limit = 100, offset = 0) {
  ...
  const headers = await withAuthHeaders();
  const res = await fetch(
    `${CHAT_API_BASE}/conversations/${conversationId}/messages?${params.toString()}`,
    { headers }
  );
  if (res.status === 401) {
    triggerAlert(
...
export async function updateChatConversation(conversationId, updates) {
  ...
  if (res.status === 401) {
    triggerAlert(
      "error",
      "Sign in required",
      "Please sign in to update conversation."
    );
    throw new Error("Unauthorized: Please sign in.");
  }
  if (res.status === 404) throw new Error("Conversation not found.");
...
export async function deleteChatConversation(conversationId) {
  ...
  if (res.status === 401) {
    triggerAlert(
      "error",
      "Sign in required",
      "Please sign in to delete conversation."
    );
    throw new Error("Unauthorized: Please sign in.");
  }
  if (res.status === 404) throw new Error("Conversation not found.");
