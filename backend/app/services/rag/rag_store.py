...
from app.services.rag.embedding_utils import get_embedding_generator, embed_query
from app.services.rag.data_ingestion_pipeline import Document
...
class RAGStore:
    ...
    async def ingest_files(self, files: List[Any], session_id: str) -> Dict[str, Any]:
        """
        Ingest uploaded files into the RAG system.
        
        Args:
            files: List of UploadFile objects from FastAPI
            session_id: Session identifier (will be created if doesn't exist)
            
        Returns:
            Dict with success status, session_id, and processing stats
        """
        try:
            # Create session if it doesn't exist
            if session_id not in self.sessions:
                # Use session_id as user_id for simplicity
                session_id = await self.create_session(user_id=session_id)
            
            # Process uploaded files
            from app.services.rag.data_ingestion_pipeline import DataIngestionPipeline
            pipeline = DataIngestionPipeline()
            
            # Convert UploadFile objects to (filename, bytes) tuples
            file_data = []
            for file in files:
            ...
            documents = await pipeline.process_files(file_data)
            
            if not documents:
                return {
                    "success": False,
                    "error": "No documents could be extracted from uploaded files",
                    "session_id": session_id,
                    "files_processed": 0,
                    "chunks_created": 0
                }
            ...
            success = await self.store_documents(session_id, documents)
            ...
            if success:
                ...
                return {
                    "success": False,
                    "error": "Failed to store documents in FAISS index",
                    "session_id": session_id,
                    "files_processed": len(files),
                    "chunks_created": 0
                }
    ...
    async def query(self, session_id: str, query: str, top_k: int = 5) -> Dict[str, Any]:
        """
        Query the RAG system and generate an answer based on retrieved documents.
        ...
        try:
            ...
            similar_docs = await self.retrieve_similar_docs(session_id, query, k=top_k)
            ...
            answer = f"Based on the uploaded documents, here are the most relevant findings:\n\n{context[:500]}..."
            
            return {
                "success": True,
                "answer": answer,
                "sources": sources,
                "confidence": float(similar_docs[0]['similarity_score']) if similar_docs else 0.0
            }
